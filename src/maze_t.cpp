...
{ 
  ...
  visited_.resize(m, n);
      ...
      //Fase 3 (redimensionamos la matriz que almacena la solucion)
      sol_.resize(m, n);
...
{ 
  if(i <= matrix_.get_m() && j <= matrix_.get_n() && i > 0 && j > 0)
...
maze_t::solve_(const int i, const int j)
{
  ...
  if(is_ok_(i - 1, j)){
    if(solve_(i - 1, j)){
      sol_.at(i - 1, j) = PATH_ID;
      return true;
    }
  }
  if(is_ok_(i, j + 1)){
    if(solve_(i, j + 1)){
      sol_.at(i, j + 1) = PATH_ID;
      return true;
    }
  }
  if(is_ok_(i + 1, j)){
